				...
				unsigned long arg4)
{
	...
	// START ACE
	// Split RAM into two, and remove the second half from the fdt
	// the second half of RAM will be used as confidential memory
	mem_offset = fdt_path_offset(fdt, "/memory");
	if (mem_offset < 0)
		goto fail;

	mem_reg_prop = fdt_getprop(fdt, mem_offset, "reg", &len);
	if (mem_reg_prop < 0 || len < 16) {
		goto fail;
	}
	// size is the second u64 number in the reg keypair <addr,size>
	// split memory in half
	new_size = cpu_to_fdt64(fdt64_ld((const fdt64_t *) &mem_reg_prop[8])/2);
	rc = fdt_setprop_inplace_namelen_partial(fdt, mem_offset, "reg", strlen("reg"), 8, &new_size, sizeof(new_size));
	if (rc < 0) {
		goto fail;
	}
	// END ACE
