...
pub fn handle(mut confidential_flow: ConfidentialFlow) -> ! {
    ...
    confidential_flow
        .transit_hart_lifecycle(HartLifecycleStateTransition::ToShutdown())
        .expect("Bug: shutdown must always succeed");
    ...
    let _ = ControlData::try_write(|control_data| control_data.remove_confidential_vm(confidential_vm_id));
