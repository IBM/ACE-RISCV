...
use crate::core::transformations::{
    ExposeToConfidentialVm, GuestStorePageFaultRequest, GuestStorePageFaultResult,
};

pub fn handle(confidential_flow: ConfidentialFlow, request: GuestStorePageFaultRequest) -> ! {
    let transformation =
        ExposeToConfidentialVm::GuestStorePageFaultResult(GuestStorePageFaultResult::new(request));
