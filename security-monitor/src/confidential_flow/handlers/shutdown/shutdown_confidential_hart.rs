...
pub fn shutdown_confidential_hart(mut confidential_flow: ConfidentialFlow) -> ! {
    ...
    let non_confidential_flow =
        confidential_flow.into_non_confidential_flow(DeclassifyToHypervisor::SbiRequest(SbiRequest::kvm_srst_system_reset()));
    ...
    // shutdown procedure.
    non_confidential_flow.exit_to_hypervisor(ExposeToHypervisor::Resume())
}
