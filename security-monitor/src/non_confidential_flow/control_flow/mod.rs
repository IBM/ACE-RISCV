...
impl<'a> NonConfidentialFlow<'a> {
    ...
    pub fn route(self) -> ! {
        ...
        use crate::non_confidential_flow::handlers::{
            esm, invalid_call, opensbi, resume, terminate, vm_hypercall,
        };
        ...
        match self.hardware_hart.trap_reason() {
            ...
            TrapReason::VsEcall(ACE_EXT_ID, ESM_FID) => {
                esm::handle(self.hardware_hart.esm_request(), self)
            }
            TrapReason::VsEcall(ACE_EXT_ID, function_id) => {
                invalid_call::handle(self, ACE_EXT_ID, function_id)
            }
            TrapReason::VsEcall(_, _) => {
                vm_hypercall::handle(self.hardware_hart.sbi_vm_request(), self)
            }
            TrapReason::HsEcall(ACE_EXT_ID, RESUME_FID) => {
            ...
            TrapReason::HsEcall(ACE_EXT_ID, function_id) => {
                invalid_call::handle(self, ACE_EXT_ID, function_id)
            }
            TrapReason::HsEcall(_, _) => {
                opensbi::handle(self.hardware_hart.opensbi_request(), self)
            }
