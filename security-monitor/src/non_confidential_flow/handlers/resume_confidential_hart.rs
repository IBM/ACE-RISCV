...
impl ResumeRequest {
    ...
    pub fn handle(self, mut non_confidential_flow: NonConfidentialFlow) -> ! {
        non_confidential_flow.restore_original_gprs();
        let (non_confidential_flow, _error) = non_confidential_flow.into_confidential_flow(self);

        // Properly implemented hypervisor should never let us enter this code. Entering this code means that the transition into
        // confidential flow failed. This might indicate an error in the hypervisor implementation because the hypervisor tried to
        // schedule an invalid confidential VM, an invalid confidential hart, or a confidential hart that is already running on
        // another physical hart. Let's keep informing the hypervisor that the confidential VM is shutdown regardless of what the
        // real reason is.
