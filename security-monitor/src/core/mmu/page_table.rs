...
use crate::core::memory_tracker::{
    ConfidentialMemoryAddress, MemoryTracker, NonConfidentialMemoryAddress, SharedPage,
};
...
impl RootPageTable {
    pub fn copy_from_non_confidential_memory(
        ...
        let page_table = PageTable::copy_from_non_confidential_memory(
            address,
            paging_system,
    ...
    pub fn map_shared_page(&mut self, shared_page: &SharedPage) -> Result<(), Error> {
        self.page_table
            .map_shared_page(self.paging_system, shared_page)
    }
...
impl PageTable {
    ...
    fn copy_from_non_confidential_memory(
        ...
        let mut page_table_memory =
            PageTableMemory::copy_from_non_confidential_memory(address, paging_system, level)?;
        let entries = page_table_memory
            ...
            .map(|index| {
                ...
                let page_table_entry = if !PageTableBits::is_valid(entry_raw) {
                    ...
                    let address =
                        NonConfidentialMemoryAddress::new(PageTableAddress::decode(entry_raw))?;
                    ...
                    let address =
                        NonConfidentialMemoryAddress::new(PageTableAddress::decode(entry_raw))?;
                    let page_table = Self::copy_from_non_confidential_memory(
                        address,
                        paging_system,
                        lower_level,
                    )?;
    ...
    fn map_shared_page(
        ...
        let virtual_page_number =
            paging_system.vpn(shared_page.confidential_vm_virtual_address(), self.level);
        let entry = self
            .entry_mut(virtual_page_number)
        ...
        match entry {
            ...
            PageTableEntry::NotValid => {
                if self.level == PageTableLevel::Level1 {
                    ...
                    next_page_table.map_shared_page(paging_system, shared_page)?;
                    let new_entry = PageTableEntry::Pointer(
                        Box::new(next_page_table),
                        PageTableConfiguration::empty(),
                    );
                    self.set_entry(virtual_page_number, new_entry);
                }
            }
