...
def process_csr_access_to_csv():
    CSR_access_results_file = open(sailor_working_dir+"/"+results_dir+"/scanner-results/riscv64/csr_access.txt", "r");
    CSR_access_results_file_lines = CSR_access_results_file.readlines();
    line_index = 0;

    csv_output_file = open(results_dir+'/scanner-results/riscv64/priv-levels-csr-access.csv', 'w', newline='');
    first_row = ["CSR", "U-mode Read", "U-mode Write", "S-mode Read", "S-mode Write", "M-mode Read", "M-mode Write"];
    ...
    while "DONE Printing CSR Access Results" not in CSR_access_results_file_lines[line_index]: 
        ...
        if cur_line in initial_ctrl_reg_names or cur_line == "frm": 
            # New CSR record. 
            m_rd = "False"; m_wr = "False"; s_rd = "False"; s_wr = "False"; u_rd = "False"; u_wr = "False";
            ...
            if u_rd == "False" and u_wr == "False" and s_rd == "False" and s_wr == "False" and m_rd == "False" and m_wr == "False": 
                ...
                if cur_line not in csrs_to_ignore: 
                    ...
                    if cur_line == "sedeleg" or "sideleg": 
                        s_rd = "True"; s_wr = "True"; m_rd = "True"; m_wr = "True";
                    else:
                        u_rd = "True"; u_wr = "True"; s_rd = "True"; s_wr = "True"; m_rd = "True"; m_wr = "True";
            
            # Finally create the row and write to the CSV 
            csr_record = [cur_line, u_rd, u_wr, s_rd, s_wr, m_rd, m_wr];
...
def arch_init(argv_sail_dir, argv_sail_model_dir, argv_results_dir): 
    ...
    print("Initializing RISC-V Arch!");
...
def arch_ctrl_reg_access():
    ...
    #os.popen('cd ../../sail-riscv/ && make csim && cd ../sailor/src/');
    #os.popen('cd ../../sail-riscv/ && ./c_emulator/riscv_sim_RV64  test/riscv-tests/rv64ui-p-add.elf > ../sailor/results/scanner-results/riscv/csr_access.txt && cd ../sailor/src/');
    global sailor_working_dir;

    sailor_working_dir = os.popen('pwd').read().replace("\n", "");

    print("Sailor working dir: "+sailor_working_dir);
    

    #os.popen('cd '+sail_dir+' && make csim && cd '+sailor_working_dir);
    #os.popen('cd '+sail_dir);
    #os.popen('cd '+sail_dir+' && ./c_emulator/riscv_sim_RV64 test/riscv-tests/rv64ui-p-add.elf > '+sailor_working_dir+'/'+results_dir+'/scanner-results/riscv64/csr_access.txt && cd '+sailor_working_dir);

    process_csr_access_to_csv(); 


def dictionaries_to_csv(user, supervisor, machine, output_file):
  """Converts three dictionaries with identical keys into a CSV file.
  """
  with open(output_file, 'w', newline='') as csvfile:
    fieldnames = ['Instruction','User Executable','Supervisor Executable','Machine Executable'];
    writer = csv.DictWriter(csvfile, fieldnames=fieldnames);
    writer.writeheader();
    for key in user.keys():
      writer.writerow({'Instruction': key, 'User Executable': user[key], 'Supervisor Executable': supervisor[key], 'Machine Executable': machine[key]});
...
def arch_instr_access():
    ...
    dictionaries_to_csv(instruction_executable[0], instruction_executable[1], instruction_executable[3], results_dir+"/scanner-results/riscv64/priv-levels-instruction-execution.csv");

    # The above doesn't generate perfect results ... Those must be manually inspected and corrected appropriately. 
    # The problem is that this Scanner doesn't track data-flow... 
    # and for e.g. if a variable is defined to check if an instruction is illegal to execute or not (e.g. sret impl in RISC-V sail model), then it'll generate incorrect results. 
    # The Scanner could be updated with more features to potentially improve some of such cases 

    # Here we will copy an annotated version of the priv-levels-instruction-execution results that was made accurate manually. 
    # For RISC-V, it is pretty simple and quick to go through such cases manually
    # Verification with Isla helps this cause further and catches any values captured incorrectly 

    os.popen('cp reference-results/scanner-results/riscv64/priv-levels-instruction-execution-annotated.csv '+results_dir+'/scanner-results/riscv64/');
