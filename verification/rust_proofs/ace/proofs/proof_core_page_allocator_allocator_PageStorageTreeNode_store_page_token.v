...
Proof.
  ...
  all: try match type of INV_WF with
    | page_storage_node_children_wf _ _ ?x => rename x into children
    end.
  ...
  - opose proof (list_lookup_lookup_total_lt children (Z.to_nat child_index) _) as Hlook_child.
    ...
    { eapply (ord_le_ord_lt_trans _ (max_node_size child)); first last.
      ...
      - done. }
    { clear. symmetry. case_bool_decide.
      - eapply max_by_l.
        apply not_ord_lt_iff. left. done.
      - eapply max_by_r. apply not_ord_gt_iff.
        destruct (option_cmp page_size_cmp x4 (Some allocable_sz_rec)) eqn:Heq; first done.
        + right. done.
  ...
  - eexists. done.
  - done.
  - (* did a merge, now it's availalbl *)
    match goal with
