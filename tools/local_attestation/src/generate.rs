...
pub fn generate_tap(
    ...
    if confidential_vm_secrets.len() >= 256 {
        return Err(Error::InvalidParameter(format!(
            "Confidential VM can receive maximum 256 secrets"
        )));
    }
    // - number of lockboxes must be lower than 1024
    if tee_public_keys_files.len() >= 1024 {
        return Err(Error::InvalidParameter(format!(
            "Confidential VM TAP support max 1024 lockboxes"
        )));
    }
    ...
    for public_key_file in tee_public_keys_files.iter() {
        ...
        let mut ciphertext =
            encrypt_rsa_2048_sha256_oasp(&symmetric_key, public_key_file.to_string())?;
    ...
    cipher
        .encrypt_in_place(&nonce, b"", &mut serialized_payload_and_hmac)
        .unwrap();
...
fn hmac_sha512(hmac_key: &[u8], payload: &[u8]) -> Result<Vec<u8>, Error> {
    use hmac::Mac;
    let mut hmac = hmac::Hmac::<sha2::Sha512>::new_from_slice(hmac_key)
        .map_err(|_| Error::InvalidSizeOfSymmetricKey())?;
    hmac.update(payload);
